...
def auth(request):
    if request.method == 'GET':
        ...
        # cria um token que dura 3600segundos
...
def  escolher_dia(request):
    ...
    if request.method == 'GET':
        ...
        disponobilidades = DisponibilidadeHorarios.objects.filter(
            data_inicial__gte=datetime.now(),
            agendado=False,
            mentor=mentorado.user
        ).values_list('data_inicial', flat=True) # coloca tudo dentro de uma lista

        datas = []
        for i in disponobilidades:
